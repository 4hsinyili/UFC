...
class UEDinerListCrawler():
    ...
    def send_location_to_UE(self, target, html_collection, responses_collection):
        ...
        self.save_html_to_mongo(html, html_collection)
        selector = etree.HTML(html)
        dict_response = self.get_diners_response(driver, responses_collection)
        self.driver.close()
        stop = time.time()
        print(stop - start)
        return selector, dict_response

    def save_html_to_mongo(self, html, collection):
        record = {'time': datetime.now(), 'html': html}
        db[collection].insert_one(record)

    def save_responses_to_mongo(self, responses, collection):
        record = {'time': datetime.now(), 'responses': responses}
        db[collection].insert_one(record)
    ...
    def get_diners_response(self, driver=None, responses_collection='responses_collection'):
        ...
        for request in driver.requests:
            if request.url == 'https://www.ubereats.com/api/getFeedV1?localeCode=tw':
                ...
                responses.extend(self.get_diner_response(diners))
        stop = time.time()
        print(stop - start)
        responses = list(set(responses))
        dict_response = {i[0]: i[1] for i in responses}
        self.save_responses_to_mongo(raw_responses, responses_collection)
...
class UEDinerDetailCrawler():
    ...
    def get_diners_details(self, data_range=0):
        ...
        diners = [self.get_diner_detail_from_UE_API(diner) for diner in diners_info]
        return diners
